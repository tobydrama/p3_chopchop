...
def run_coefficient_score(stranded_guide_seq: str) -> float:
    ...
    **Docker wrapper for CRISPRoff.**
    ...
    WARNING: this function may raise an exception if the main python script does not have either 'docker' group
    permissions, or root (sudo) permissions; docker requires either group or root permissions to run.

    :param stranded_guide_seq: The input sequence for CRISPRoff_score()
    :return: A float value. Coefficient score maybe?
    :raises subprocess.CalledProcessError: if docker returns a non-zero exit value.
    """
    command = ['docker', 'run', 'chopchop_crisproff', stranded_guide_seq]

    """Error handling is done by subprocess.run flag 'check=True',
    if the docker container returns a non-zero return value,
